...
- (instancetype)initWithTagViewHeight:(CGFloat)tagViewHeight
{
    if (self = [super init]) {
        ...
        [self setSeartchButton];
    }
    return self;
}
-(void)setSeartchButton
{
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(LYSW - 60, 20, 60, 44)];
    [button setImage:[UIImage imageNamed:@"bar_btn_icon_search"] forState:UIControlStateNormal];
    [button addTarget:self action:@selector(seartchAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:button];
}
-(void)seartchAction:(UIButton*)sender
{
...
- (void)viewDidLayoutSubviews
{
    ...
    self.tagCollectionView.frame = CGRectMake(0, 20, XBScreenWidth - 60, self.tagViewHeight);
    self.pageCollectionView.frame = CGRectMake(0, self.tagViewHeight+20, XBScreenWidth, self.view.frame.size.height - self.tagViewHeight-20);
    [self.pageCollectionView reloadData];
//    NSLog(@"+++++++++++%@",NSStringFromCGSize(self.pageCollectionView.size));
}
...
//    pageFlowLayout.sectionInset = UIEdgeInsetsMake(0, 0, 0, 0);
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    {
//        NSLog(@"===%@",NSStringFromCGSize(collectionView.size));
